...
object Histogram {
  ...
  /** Persist a timed value into this [[Histogram]]
    ...
    */
  def timed[F[_]: Clock, A](h: Histogram[F], fa: F[A], unit: TimeUnit)(implicit
      C: MonadCancel[F, _]
  ): F[A] =
  ...
  /** Persist a timed value into this [[Histogram]] in unit Seconds. This is exposed.
    ...
    * @param fa The action to time
    */
  def timedSeconds[F[_]: Clock, A](h: Histogram[F], fa: F[A])(implicit
      C: MonadCancel[F, _]
  ): F[A] =
    timed(h, fa, SECONDS)
  ...
  /** Constructor for a [[Histogram]] with no labels. Default buckets are [[defaults]]
    ...
    *
    * @param pr PrometheusRegistry this [[Histogram]] will be registered with
  ...
  def noLabels[F[_]: Sync](
    ...
    noLabelsBuckets(pr, name, help, defaults: _*)
  ...
  /** Constructor for a [[Histogram]] with no labels. Default buckets are [[defaults]]
    ...
    * @param help The help string of the metric
    * @param upperBounds The buckets to measure observations by.
    */
  def noLabelsBuckets[F[_]: Sync](
    ...
    c <- Sync[F].delay(
      JHistogram
        ...
        .name(name.getName)
        .help(help)
        .classicUpperBounds(upperBounds: _*)
    )
  ...
  def noLabelsLinearBuckets[F[_]: Sync](
    ...
    c <- Sync[F].delay(
      JHistogram
        ...
        .name(name.getName)
        .help(help)
        .classicLinearUpperBounds(start, width, count)
    )
    out <- Sync[F].delay(c.register(PrometheusRegistry.Unsafe.asJava(pr)))
  } yield new NoLabelsHistogram[F](out)
  ...
  /** Constructor for a labelled [[Histogram]]. Default buckets are [[defaults]]
    ...
    *
    * @param pr PrometheusRegistry this [[Histogram]] will be registred with
    * @param name The name of the [[Histogram]].
    * @param help The help string of the metric
  ...
  def labelled[F[_]: Sync, A, N <: Nat](
    ...
    labelledBuckets(pr, name, help, labels, f, defaults: _*)
  ...
  private final class MapKHistogram[F[_], G[_]](
  ...
  sealed trait UnlabelledHistogram[F[_], A] {
    ...
    def mapK[G[_]](fk: F ~> G): UnlabelledHistogram[G, A] =
      new MapKUnlabelledHistogram[F, G, A](this, fk)
  }

  final private[epimetheus] class UnlabelledHistogramImpl[
      F[_]: Sync,
      A
  ] private[epimetheus] (
      private[Histogram] val underlying: JHistogram,
    ...
    def label(a: A): Histogram[F] =
      new LabelledHistogram[F](underlying.labelValues(f(a): _*))
  }

  final private class MapKUnlabelledHistogram[F[_], G[_], A](
      private[Histogram] val base: UnlabelledHistogram[F, A],
      fk: F ~> G
  ) extends UnlabelledHistogram[G, A] {
  ...
  object Unsafe {
    ...
    def fromJava[F[_]: Sync](h: DistributionDataPoint): Histogram[F] =
      new LabelledHistogram(h)
    def fromJavaUnlabelled[F[_]: Sync](h: JHistogram): Histogram[F] =
      new NoLabelsHistogram(h)
  }
