...
trait SummaryCommons {
  ...
  /** Persist a timed value into this [[Summary]]
    ...
    */
  def timed[F[_]: Clock, A](s: Summary[F], fa: F[A], unit: TimeUnit)(implicit
      C: MonadCancel[F, _]
  ): F[A] =
    C.bracket(Clock[F].monotonic)((_: FiniteDuration) => fa) {
  ...
  /** Persist a timed value into this [[Summary]] in unit Seconds. Since the default
    ...
    * @param fa The action to time
    */
  def timedSeconds[F[_]: Clock, A](s: Summary[F], fa: F[A])(implicit
      C: MonadCancel[F, _]
  ): F[A] =
    timed(s, fa, SECONDS)
  ...
  /** Default Constructor for a [[Summary]] with no labels.
    ...
    *
    * @param pr PrometheusRegistry this [[Summary]] will be registered with
  ...
  def noLabelsQuantiles[F[_]: Sync](
    ...
    c <- Sync[F].delay(quantiles.foldLeft(c1) { case (c, q) =>
      c.quantile(q.quantile, q.error)
    })
  ...
  /** Default Constructor for a labelled [[Summary]].
    ...
    *
    * @param pr PrometheusRegistry this [[Summary]] will be registred with
    * @param name The name of the [[Summary]].
    * @param help The help string of the metric
  ...
  def labelledQuantiles[F[_]: Sync, A, N <: Nat](
    ...
    c1 <- Sync[F].delay(
      JSummary
        ...
        .help(help)
        .maxAgeSeconds(maxAgeSeconds)
        .numberOfAgeBuckets(numberOfAgeBuckets)
  ...
  final private[epimetheus] class NoLabelsSummary[
      F[_]: Sync
  ] private[epimetheus] (
  ...
  final private[epimetheus] class LabelledSummary[
      F[_]: Sync
  ] private[epimetheus] (
      private[epimetheus] val underlying: DistributionDataPoint
  ) extends Summary[F] {
    def observe(d: Double): F[Unit] = Sync[F].delay(underlying.observe(d))
  ...
  final private[epimetheus] class MapKSummary[F[_], G[_]](
      private[epimetheus] val base: Summary[F],
      fk: F ~> G
  ) extends Summary[G] {
  ...
  sealed trait UnlabelledSummary[F[_], A] {
    ...
    def mapK[G[_]](fk: F ~> G): UnlabelledSummary[G, A] =
      new MapKUnlabelledSummary[F, G, A](this, fk)
  }
  final private[epimetheus] class UnlabelledSummaryImpl[
      F[_]: Sync,
      A
  ] private[epimetheus] (
      private[epimetheus] val underlying: JSummary,
    ...
    def label(a: A): Summary[F] =
      new LabelledSummary[F](underlying.labelValues(f(a): _*))
  }

  final private[epimetheus] class MapKUnlabelledSummary[F[_], G[_], A](
      private[epimetheus] val base: UnlabelledSummary[F, A],
      fk: F ~> G
  ) extends UnlabelledSummary[G, A] {
  ...
  trait QuantileCommons {
    ...
    def impl(
        quantile: Double,
        error: Double
    ): Either[IllegalArgumentException, Summary.Quantile] = {
    ...
    def implF[F[_]: ApplicativeThrow](
        quantile: Double,
        error: Double
    ): F[Summary.Quantile] =
  ...
  object Unsafe {
    ...
    def fromJava[F[_]: Sync](s: DistributionDataPoint): Summary[F] =
      new LabelledSummary(s)
    def fromJavaUnlabelled[F[_]: Sync](s: JSummary): Summary[F] =
      new NoLabelsSummary(s)
  }
}
