...
object Counter {
  ...
  private final class MapKCounter[F[_], G[_]](base: Counter[F], fk: F ~> G) extends Counter[G]{
    def get: G[Double] = fk(base.get)
    def inc: G[Unit] = fk(base.inc)
    def incBy(d: Double): G[Unit] = fk(base.incBy(d))
  }

  trait UnlabelledCounter[F[_], A]{
   ...
   */
  private class UnlabelledCounterImpl[F[_]: Sync, A] private[Counter](
  ...
  private class MapKUnlabelledCounter[F[_], G[_], A](base: UnlabelledCounter[F, A], fk: F ~> G) extends UnlabelledCounter[G, A]{
