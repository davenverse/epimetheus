...
final class Summary[F[_]: Sync] private (private val s: JSummary.Child){
  def observe(d: Double): F[Unit] = Sync[F].delay(s.observe(d))
}

object Summary {
  def buildQuantiles[F[_]: Sync](cr: CollectorRegistry, name: String, help: String, quantiles: (Double, Double)*): F[Summary[F]] = for {
  ...
  final class SafeUnlabelledSummary[F[_]: Sync, A] private[epimetheus](
    private val c: JSummary, 
    private val f: A => List[String]
  ) {
